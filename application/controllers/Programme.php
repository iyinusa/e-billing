...
class Programme extends CI_Controller {
	...
	public function index() {
		...
		// load records
		$list = '';
		$allrec = $this->Crud->read('eb_programme');
		if(!empty($allrec)){
			foreach($allrec as $rec){
				...
				$edit_link = '<li><a href="javascript:;" class="pop text-primary" pageTitle="Edit '.$name.'" pageName="'.base_url('programme/form/add/'.$id).'"><i class="si si-pencil"></i> Manage Record</a></li>';
				
				$del_link = '<li><a href="javascript:;" class="pop text-danger" pageTitle="Delete '.$name.'" pageName="'.base_url('programme/form/del/'.$id).'"><i class="si si-trash"></i> Delete Record</a></li>';
	...
	public function form($param1='',$param2='') {
		...
		if($param1 == 'add' || $param1 == ''){
			if($param2 != '') {
				$getrec = $this->Crud->read_single('id', $param2, 'eb_programme');
				if(!empty($getrec)){
			...
			if($_POST){
				...
				if($prog_id != ''){
					...
					$upd_id = $this->Crud->update('id', $prog_id, 'eb_programme', $upd_data);
					if($upd_id){
						$err_msg = $this->Crud->msg('success', 'Record Updated');
						
						echo '<script>window.location.replace("'.base_url('programme').'");</script>';	
					} else {
						$err_msg .= $this->Crud->msg('info', 'No Record Changes');
					}
				} else {
					if($this->Crud->check('name', $name, 'eb_programme') > 0){
						...
						$ins_id = $this->Crud->create('eb_programme', $ins_data);
						if($ins_id){
		...
		//////////////// DELETE RECORDS /////////////
			...
			if($_POST){
				$d_prog_id = $_POST['d_prog_id'];
				if($_POST){
					if($this->Crud->delete('id', $d_prog_id, 'eb_programme') > 0) {
