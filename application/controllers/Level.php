...
class Level extends CI_Controller {
	...
	public function index() {
		...
		// load records
		$list = '';
		$allrec = $this->Crud->read_order('eb_level', 'name', 'asc');
		if(!empty($allrec)){
			foreach($allrec as $rec){
				$id = $rec->id;
				$name = $rec->name;
				
				$edit_link = '<li><a href="javascript:;" class="pop text-primary" pageTitle="Edit '.$name.'" pageName="'.base_url('level/form/add/'.$id).'"><i class="si si-pencil"></i> Manage Record</a></li>';
				
				$del_link = '<li><a href="javascript:;" class="pop text-danger" pageTitle="Delete '.$name.'" pageName="'.base_url('level/form/del/'.$id).'"><i class="si si-trash"></i> Delete Record</a></li>';
	...
	public function form($param1='',$param2='') {
		...
		if($param1 == 'add' || $param1 == ''){
			if($param2 != '') {
				$getrec = $this->Crud->read_single('id', $param2, 'eb_level');
				if(!empty($getrec)){
			...
			if($_POST){
				...
				if($level_id != ''){
					...
					$upd_id = $this->Crud->update('id', $level_id, 'eb_level', $upd_data);
					if($upd_id){
						$err_msg = $this->Crud->msg('success', 'Record Updated');
						
						echo '<script>window.location.replace("'.base_url('level').'");</script>';	
					} else {
						$err_msg .= $this->Crud->msg('info', 'No Record Changes');
					}
				} else {
					if($this->Crud->check('name', $name, 'eb_level') > 0){
						...
						$ins_id = $this->Crud->create('eb_level', $ins_data);
						if($ins_id){
		...
		//////////////// DELETE RECORDS /////////////
			...
			if($_POST){
				$d_level_id = $_POST['d_level_id'];
				if($_POST){
					if($this->Crud->delete('id', $d_level_id, 'eb_level') > 0) {
