...
class Department extends CI_Controller {
	...
	public function index() {
		...
		// load records
		$list = '';
		$allrec = $this->Crud->read('eb_department');
		if(!empty($allrec)){
			foreach($allrec as $rec){
				...
				$school_name = $this->Crud->read_field('id', $rec->school_id, 'eb_school', 'name');
				
				$edit_link = '<li><a href="javascript:;" class="pop text-primary" pageTitle="Edit '.$name.'" pageName="'.base_url('department/form/add/'.$id).'"><i class="si si-pencil"></i> Manage Record</a></li>';
				
				$del_link = '<li><a href="javascript:;" class="pop text-danger" pageTitle="Delete '.$name.'" pageName="'.base_url('department/form/del/'.$id).'"><i class="si si-trash"></i> Delete Record</a></li>';
		...
		$data['title'] = app_name.' | Department';
	...
	public function form($param1='',$param2='') {
		...
		if($param1 == 'add' || $param1 == ''){
			if($param2 != '') {
				$getrec = $this->Crud->read_single('id', $param2, 'eb_department');
				if(!empty($getrec)){
					foreach($getrec as $rec){
						$data['e_id'] = $rec->id;
						$data['e_school_id'] = $rec->school_id;
			...
			if($_POST){
				...
				if($department_id != ''){
					...
					$upd_id = $this->Crud->update('id', $department_id, 'eb_department', $upd_data);
					if($upd_id){
						$err_msg = $this->Crud->msg('success', 'Record Updated');
						
						echo '<script>window.location.replace("'.base_url('department').'");</script>';	
					} else {
						$err_msg .= $this->Crud->msg('info', 'No Record Changes');
					}
				} else {
					if($this->Crud->check('name', $name, 'eb_department') > 0){
						...
						$ins_id = $this->Crud->create('eb_department', $ins_data);
						if($ins_id){
		...
		//////////////// DELETE RECORDS /////////////
			...
			if($_POST){
				$d_department_id = $_POST['d_department_id'];
				if($_POST){
					if($this->Crud->delete('id', $d_department_id, 'eb_department') > 0) {
